...
namespace SHARED_PS2_BIN.REPACK
{
    ...
    {
        private static byte[] MakeBonePairArry((ushort b1, ushort b2, ushort b3, ushort b4)[] bonepairLines) 
        {
            if (bonepairLines.Length != 0)
            {
                ...
                {
                    BitConverter.GetBytes((ushort)bonepairLines[i].b1).CopyTo(res, offset);
                    BitConverter.GetBytes((ushort)bonepairLines[i].b2).CopyTo(res, offset + 2);
                    BitConverter.GetBytes((ushort)bonepairLines[i].b3).CopyTo(res, offset + 4);
                    BitConverter.GetBytes((ushort)bonepairLines[i].b4).CopyTo(res, offset + 6);
                    offset += 8;
                }
                return res;
            }
        ...
        public static void MakeFinalBinFile(Stream binfile, long startOffset, out long endOffset, FinalStructure finalStructure, RepackProps props, FinalBoneLine[] bones, float ConversionFactorValue, IdxMaterial material) 
        {
            ...
            {
                //MakeNodeArray
                nodesBytes.Add((item.MaterialName, MakeNodeArray(item, ConversionFactorValue, props.IsScenarioBin)));
            }
            ...
            byte[] bonepairArray = MakeBonePairArry(props.BonePairs);
            ...
            {
                version_flags = 0x20030818;
            }
            else
            {
                version_flags = 0x20010801;
            }

            if (props.EnableUnkFlag1)
            {
                binFlags |= (uint)BinFlags.EnableUnkFlag1;
            }

            if (props.EnableUnkFlag2)
            {
                binFlags |= (uint)BinFlags.EnableUnkFlag2;
            }

            if (props.EnableUnkFlag4)
            {
                binFlags |= (uint)BinFlags.EnableUnkFlag4;
            }

            BitConverter.GetBytes(version_flags).CopyTo(binHeader, 0x18);
            BitConverter.GetBytes(binFlags).CopyTo(binHeader, 0x24);

            binHeader[0x28] = 0x30;

            BitConverter.GetBytes(props.BoundingBox.BoundingBoxPosX).CopyTo(binHeader, 0x30);
            BitConverter.GetBytes(props.BoundingBox.BoundingBoxPosY).CopyTo(binHeader, 0x34);
            BitConverter.GetBytes(props.BoundingBox.BoundingBoxPosZ).CopyTo(binHeader, 0x38);
            BitConverter.GetBytes(props.BoundingBox.BoundingBoxPosW).CopyTo(binHeader, 0x3C);
            BitConverter.GetBytes(props.BoundingBox.BoundingBoxWidth).CopyTo(binHeader, 0x40);
            BitConverter.GetBytes(props.BoundingBox.BoundingBoxHeight).CopyTo(binHeader, 0x44);
            BitConverter.GetBytes(props.BoundingBox.BoundingBoxDepth).CopyTo(binHeader, 0x48);
